...
To run `mpegCoder`, you are required to install [Numpy<IconExternalLink/>][link-numpy] with the correct version first. The best version for each `mpegCoder` release has been listed before. If your Numpy version is differnt from the best version too much, `mpegCoder` may not work.
...
| FFMpeg | GCC/G++ | OS |
| :-----: | :-----: | :-----: |
| [`4.4`<InlineIcon icon={mdiDownloadBox}/>][download-ff-4-4] | `9.3.0` | `20.04.2` |
...
Running the pre-compiled `mpegCoder` requires users to add the required dynamic libraries to your path. The extracted dependency files should contain two folders:
