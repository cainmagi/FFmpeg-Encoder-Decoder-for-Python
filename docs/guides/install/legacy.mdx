...
The following built are legacy versions. They are not technically supported now. But they support older FFMpeg versions. Note that not all funcionalities of `mpegCoder` are supported in these versions. They may also contains severe bugs.
